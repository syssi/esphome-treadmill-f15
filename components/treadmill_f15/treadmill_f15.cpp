  ...
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ...
      if (char_notify == nullptr) {
        ESP_LOGE(TAG, "[%s] No notify service found at device, not an Treadmill BMS..?", ADDR_STR(this->parent_->address_str()));
        break;
      }
      ...
      if (char_command == nullptr) {
        ESP_LOGE(TAG, "[%s] No control service found at device, not an Treadmill BMS..?", ADDR_STR(this->parent_->address_str()));
        break;
      }
...
void TreadmillF15::dump_config() {  // NOLINT(google-readability-function-size,readability-function-size)
  ...
  LOG_NUMBER("", "Speed Control", this->speed_number_);
  LOG_NUMBER("", "Incline Control", this->incline_number_);
...
void TreadmillF15::publish_state_(number::Number *number, float value) {
  if (number == nullptr)
    return;
...
bool TreadmillF15::send_command(uint8_t register_addr, uint8_t payload) {
  std::vector<uint8_t> command = {0x02, register_addr, payload};
  uint8_t crc_value = crc(command.data() + 1, command.size() - 1);
...
bool TreadmillF15::send_command(uint8_t register_addr, const std::vector<uint8_t> &payload) {
  std::vector<uint8_t> command = {0x02, register_addr};
...
bool TreadmillF15::send_speed_incline_command(float speed, float incline) {
  // Command structure: [0x02, 0x53, 0x02, speed*10, incline, ...padding..., crc, 0x03]

  uint8_t speed_value = (uint8_t) (speed * 10);  // Convert km/h to 0.1 km/h units
  uint8_t incline_value = (uint8_t) incline;     // Incline as integer percentage

  std::vector<uint8_t> command = {
      0x02,          0x53, 0x02,                         // Start frame, register 0x53, command 0x02
      speed_value,                                       // Speed * 10
      incline_value,                                     // Incline %
